...
layui.use('table', function(){
  ...
  ,url:"<%=basePath%>attendanceController/query/All"
    ...
    ,cols: [[
             ...
             ,{field:'attenState', title:'打卡状态', width:120,  templet: function(res){
            	 var staus;
            	 if(res.attenState==1){
            		 staus="<i class='layui-icon layui-icon-ok' style='font-size: 20px; color: #2ec770;'></i> ";
            	 }else{
            		 staus="<i class='layui-icon layui-icon-close' style='font-size: 20px; color: #ff4100;'></i>  ";
            	 }
                 return '<em>'+  
                 staus
                 +'</em>'
    ...
    if(obj.event === 'del'){
      layer.confirm('确认删除？', function(index){
        ...
        layer.close(index);
          ...
          layer.close(index);						//问题这个东西在这好像没有，因为绑定的是del的
      }
                ...
                success:function (data) {
              	  if(data==1){
             	      obj.update({
              		  ...
              		  layer.msg('打卡登记成功！',{anim: 0});
              		 // alert("删除成功！");
              	  }else{
              		layer.msg('打卡登记失败！',{anim: 6});
              	  }
                },									//响应成功后的回调函数
                error:function () {
                    alert("出错啦...")
